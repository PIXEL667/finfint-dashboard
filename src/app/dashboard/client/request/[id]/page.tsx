...
const initialPayments: Payment[] = [
    { id: "p1", service_request_id: "sr002", amount: 1178.82, payment_method: "UPI", payment_status_id: "ps_success", payment_status: "completed", transaction_id: "pay_abc123xyz456", reference_number: "order_def789ghi012", created_at: "2026-01-08T14:30:00Z" },
    { id: "p2", service_request_id: "sr004", amount: 708, payment_method: "Card", payment_status_id: "ps_success", payment_status: "completed", transaction_id: "pay_mno345pqr678", reference_number: "order_stu901vwx234", created_at: "2025-12-14T16:00:00Z" },
];
...
export default function RequestDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (request.status === "pending") badgeClass = "bg-yellow-100 text-yellow-700";
    if (request.status === "in_progress") badgeClass = "bg-primary/10 text-primary";
    if (request.status === "completed") badgeClass = "bg-accent/15 text-accent-foreground";
    ...
    const handlePayment = () => {
        ...
        setTimeout(() => {
            ...
            toast.success(
                `Payment successful! ${formatCurrency(request.total_amount_snapshot)} paid. TXN: ${txnId}`
            );
        }, 1500);
    };
            ...
            <div className="bg-card rounded-xl border border-border p-6 shadow-sm space-y-5">
                        ...
                        >
                            {isProcessingPayment ? (
                                ...
                                <>
                                    ...
                                    Pay {formatCurrency(request.total_amount_snapshot)}
                                </>
                            )}
                        </Button>
                    )}
                ...
                {payments.length > 0 ? (
                            ...
                            return (
                                <div key={p.id} className="rounded-lg border border-border p-4 bg-muted/20">
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        ...
                                        <div>
                                            <p className="text-xs uppercase tracking-wider font-medium text-muted-foreground mb-1">Date</p>
                                            <p className="font-medium text-sm">{formatDate(p.created_at)}</p>
                                        </div>
                                    </div>
                    ...
                    <div className="py-4 text-center">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">Click the pay button above to pay via Razorpay.</p>
                    </div>
                )}
            </div>
